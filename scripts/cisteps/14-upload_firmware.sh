...
function get_firmware_hash(){
	grep "openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-$1-sysupgrade.img.gz" "sha256sums" | awk -F ' ' '{print $1}' 2>"/dev/null"
}

function transfer_upload(){
	file_name="openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-$2-sysupgrade.img.gz"
	if [ "$1" == "cow" ]; then
